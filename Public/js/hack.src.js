...
(function() {
	function isiOS() {
		...
		return p.match(/iPhone/i) || p.match(/iPad/i)
	}

	function isWxNewVersion() {
		var a = navigator.userAgent.match(/MicroMessenger\/([\d\.]+)/i);
		if (!a || a.length <= 0) {
			return false
		}
		var b = a[1].split(".");
		...
		var c = parseInt(b[0]) * 1000 * 1000 + parseInt(b[1]) * 1000 + parseInt(b[2]);
		return c >= 6003023
	}

	function shareCallback(a) {
		var b = a['err_msg'];
		if (b == 'send_app_msg:ok' || b == 'send_app_msg:confirm') {
			if (window.shareobject.shareComplete) window.shareobject.shareComplete(0)
		} else if (b == 'share_timeline:ok' || b == 'share_timeline:confirm') {
			if (window.shareobject.shareComplete) window.shareobject.shareComplete(1)
		} else if (b == 'share_timeline:canecl') {
			if (window.shareobject.shareCancel) window.shareobject.shareCancel(1)
		} else if (b == 'send_app_msg:canecl') {
			if (window.shareobject.shareCancel) window.shareobject.shareCancel(0)
		}
	}
	var m = false;
	window.WeixinJSBridge_rebuild = undefined;
	window.WeixinJSBridge_copy = undefined;
	window.shareobject = {
		getshareData: undefined,
		shareComplete: undefined,
		shareCancel: undefined
	};
	Object.defineProperty(window, 'WeixinJSBridge', {
		get: function() {
			if (window.WeixinJSBridge_rebuild) {
				return window.WeixinJSBridge_rebuild
			}
			return window.WeixinJSBridge_copy
		},
		set: function(l) {
			window.WeixinJSBridge_copy = l;
			window.WeixinJSBridge_rebuild = {};
			try {
				window.WeixinJSBridge_rebuild.invoke = window.WeixinJSBridge_copy.invoke
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild.call = window.WeixinJSBridge_copy.call
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild.on = window.WeixinJSBridge_copy.on
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild.env = window.WeixinJSBridge_copy.env
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild.log = window.WeixinJSBridge_copy.log
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._getSelectedText = window.WeixinJSBridge_copy._getSelectedText
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._fetchQueue = window.WeixinJSBridge_copy._fetchQueue
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._handleMessageFromWeixin = function(a) {
					if (isWxNewVersion()) {
						window.WeixinJSBridge_copy._handleMessageFromWeixin(a)
					}
					var b = a['__json_message'];
					var c = a['__sha_key'];
					var d = b['__event_id'];
					var e = b['__msg_type'];
					var f = b['__callback_id'];
					if (e == 'callback' && f) {
						var g = b['__params'];
						shareCallback(g)
					}

					if (d && d.indexOf("share") > 0) {
						var h = "sendAppMessage";
						var i = d;
						if (d == "general:share") {
							var g = b['__params'];
							i = g['shareTo']
						}
						if (i.indexOf("timeline") != -1) {
							h = "shareTimeline"
						}
						if (h) {
							window.shareType = h;
							if (typeof(window.shareobject.getshareData) != 'function') {
								return
							}
							var j = window.shareobject.getshareData();
							var k = function() {};
							if (h == "shareTimeline") {
								window.WeixinJSBridge_rebuild.invoke(h, j, k)
							} else {
								isWxNewVersion() ? setTimeout(function() {
									window.WeixinJSBridge_rebuild.invoke(h, j, k)
								}, 300) : window.WeixinJSBridge_rebuild.invoke(h, j, k), setTimeout(function() {
									window.WeixinJSBridge_rebuild.invoke(h, j, k)
								}, 350)
							}
							reloadShareHandle()
			...
			try {
				window.WeixinJSBridge_rebuild._hasInit = window.WeixinJSBridge_copy._hasInit
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._hasPreInit = window.WeixinJSBridge_copy._hasPreInit
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._isBridgeByIframe = window.WeixinJSBridge_copy._isBridgeByIframe
			} catch (w) {}
			try {
				window.WeixinJSBridge_rebuild._continueSetResult = window.WeixinJSBridge_copy._continueSetResult
			} catch (w) {}
			m = true
		}
	})
